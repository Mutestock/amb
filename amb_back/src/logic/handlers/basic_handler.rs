...
pub async fn check_conn_string() -> Result<impl warp::Reply, warp::Rejection> {
    let db_url_one =
        env::var("DATABASE_URL").expect("No DATABASE_URL environment variable was found...");

    dotenv().ok();

    let db_url_two =
        env::var("DATABASE_URL").expect("No dotenv DATABASE_URL environment variable was found...");

    Ok(format!(
        "Conn pre: {}, post conn: {}",
        db_url_one, db_url_two
    ))
}
