...
pub async fn upload(form: FormData) -> Result<impl Reply, Rejection> {
    let parts: Vec<Part> = form.try_collect()
    .await.
    map_err(|e| {
        ...
        if p.name() == "file" {
            ...
            match content_type {
                        ...
                        match file_name {
                            Some(f_name) => match f_name.split(".").collect::<Vec<&str>>()[1]{
                                ...
                                z => {  
                                    ...
                                    return Err(warp::reject::custom(error_handling::Error::InvalidFileTypeError))
                                }
                            },
                            None => {
                                eprintln!("No file name for uploaded file. Upload failure");
                                return Err(warp::reject::custom(error_handling::Error::MissingFileNameError))
                            }
                        };
                    }
                },
                None => {
                    ...
                    return Err(warp::reject::custom(error_handling::Error::FileTypeDeterminationError))
                }
            }
...
//
            ...
            let root_resource_path = env::var("RESOURCE_PATH")
                .expect("RESOURCE_PATH environment variable must be set.");
            ...
            let new_track: NewTrack = new_track_reception.to_new_track(_uuid.clone(), resource_path.clone().to_string());   
            let response = new_track.create(&conn);

            fs::create_dir_all(format!("{}/{}",&resource_path, &username))
