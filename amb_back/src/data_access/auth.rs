...
pub fn create_jwt(user_response: UserResponse) -> Result<String, Error> {
    ...
    encode(&header, &claims, &EncodingKey::from_secret(get_jwt_secret().as_bytes()))
        .map_err(|_| Error::JWTTokenCreationError)
}

pub fn with_auth(user_response: UserResponse) -> impl Filter<Extract = (UserResponse,), Error = Rejection> + Clone {
    headers_cloned()
        .map(move |headers: HeaderMap<HeaderValue>| (user_response.clone(), headers))
        .and_then(authorize)
}

async fn authorize((user_response, headers):(UserResponse, HeaderMap<HeaderValue>)) -> Result<UserResponse, Rejection> {
